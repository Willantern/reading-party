...
<div style="text-align: center; margin: 10px 0;">
  <img src="/images/wearable-devices.png" 
...
“Despite wearable self-tracking devices’ potential to improve their users’ health… To yield positive health effects, wearables must not only be used but also used effectively, that is, in a way that helps achieve the goals for using them.”(Abstract)

“A broad swath of the population uses wearable self-tracking devices (“wearables” hereafter) to improve a variety of health-related behaviors, including exercising and eating healthfully, by leveraging their agentic properties and persuasive features (Abouzahra and Ghasemaghaei 2021).”(1. Introduction)

“Empirical studies show that wearables help to improve health outcomes (Brickwood et al. 2019), and the health sector places considerable confidence in the technology’s ability to mitigate the detrimental effects of an unhealthful lifestyle (World Health Organization 2014).”(1. Introduction)

“The hope for wearables’ ability to promote healthful behaviors is exemplified in healthcare practitioners’ choosing to incorporate wearables into therapy plans (Hiremath et al. 2014) and health insurance companies’ offers of discounted premiums to users of wearables (Wiegard and Breitner 2019).”(1. Introduction)
...
然而，现实往往有些尴尬：
## **高弃用率(短期停用)**
很多人在佩戴几个月后，设备就沦为了抽屉里的收藏品。
## **无效使用(未发挥健康价值)**
虽然戴着设备，但我们并没有因为它的数据而改变行为；甚至为了凑步数而“摇晃手环”，为了数据好看而只记录“拿得出手”的运动成绩。
## **负面体验频发(用户情绪与心理负担)**
当设备提示你“站起来走走”时，你可能感到被监视或厌烦；当GPS定位不准导致配速变慢时，你可能感到挫败 。
...
“they do not live up to that potential when users discontinue using them after short periods or use them in ineffective ways.”(Abstract)

“research and practitioner reports provide evidence of high attrition rates (Gartner 2016; Attig and Franke 2020, 2022; Farr 2020), failure to take full advantage of the wearables’ health-promoting value (Benbunan-Fich 2019, Attig and Franke 2022), and negative experiences that make users feel guilty, pressured, frustrated, or surveilled (de Moya and Pallud 2020, Rieder et al. 2021, Constantiou et al. 2022).”(1. Introduction)

“high abandonment rates that occur after only short periods of use (Gartner 2016, Farr 2020) due to declining motivation, low perceptions of wearables’ benefits, obsessive tracking, and privacy concerns (Matt et al. 2019; Attig and Franke 2020, 2022).”(1. Introduction)

“negative incidents that occur while using wearables, such as stress from negative feedback (Constantiou et al. 2022), pressure and frustration from inflexible schedules or comparison with peers (Rieder et al. 2021), and feelings of being surveilled (de Moya and Pallud 2020).”(1. Introduction)

“dysfunctional and ineffective use, such as use that is motivated by physical appearance, which is negatively associated with subjective well-being (James et al. 2019), long-term ineffective use that is due to lack of motivation to adhere to cues despite looking at the collected data frequently (Lehrer et al. 2021), and perceptions of unsatisfactory data as personal failure, which requires users to deal with adversity (Constantiou et al. 2022).”(1. Introduction)
...
## **能动性冲突 (Conflict)**
你只想记录数据，设备却强迫你完成它设定的目标(Forced Goals)。
...
She has taught courses on digital transformation, IT strategy, IT design and use, behavioral experiments, and “nudging” at the undergraduate, graduate, and executive level. She also advises diverse organizations, including government agencies and multinational corporations, guiding them on the effective and ethical utilization of persuasive systems.
...
Although the literature on what can help to ensure effective use is conclusive, little is known about the dynamics that are at play once effective use is established.”(Abstract)

“The effective use of organizational applications (Liang et al. 2015, Torres and Sidorova 2019, Bao et al. 2020), the antecedents of establishing effective use (Hornyak et al. 2020, Savoli et al. 2020, Tams et al. 2020), and the behavioral dimensions of effective use (Burton-Jones and Volkoff 2017, Trieu et al. 2022) are well understood. However, because most of the literature on the topic focuses on how effective use is established, what happens to established effective use when negative incidents arise and how users cope with them require further exploration.”(1. Introduction)

“The vast information systems (IS) literature on coping (D’Arcy et al. 2014, Stein et al. 2015, Pirkkalainen et al. 2019, Salo et al. 2020) focuses on how users appraise stressful situations when they use IS (Beaudry and Pinsonneault 2005, Liang and Xue 2009, Stein et al. 2015) and how they choose to cope (Liang et al. 2019, Salo et al. 2020) but gives limited attention to the outcomes of coping (Beaudry and Pinsonneault 2010, Wang et al. 2017, Pirkkalainen et al. 2019) even though only outcomes indicate the success of coping.”(1. Introduction)

“This study seeks to shed light on coping outcomes with respect to effective use by addressing the following research question: How does coping with negative incidents affect users’ effective use of wearables?”(1. Introduction)
...
# **叙事访谈(Narrative Interviews)**
研究采用 “叙事访谈” 作为核心数据收集手段，原因在于该方法能获取用户深度、主观的个人经历叙事(Myers 1997)—— 用户可按自己的逻辑和结构讲述使用可穿戴设备的完整故事，尤其适合捕捉 “负面事件发生→情绪反应→应对行为→后续使用变化” 的序列性过程(这与 “应对理论” 中 “ coping as a process” 的核心逻辑一致，Lazarus 1993)。

# **理由**
“Narrative interviews are particularly suitable for obtaining data on a past event that has a sequential structure, which was useful in the present study’s efforts to extract users’ ongoing use of wearables, the impact of negative incidents, and how users reacted to these incidents...”，且该方法在 IS 领域的压力与应对研究中已被验证有效(如 Stein et al. 2015；Salo et al. 2018, 2022)。

# **访谈逻辑**

为避免预设框架干扰用户表达，研究采用 “开放式刺激问题 + 深度追问” 的引导模式：
- **核心刺激问题**

以中性、宽泛的问题开启叙事，如 “Please tell me the story of your wearable, from how it started until today”(请讲讲你的可穿戴设备故事，从开始使用到现在)，让用户自主梳理使用历程；

- **负面事件追问**

在用户初步叙事后，进一步引导聚焦负面事件，如 “Please share the negative incidents you encountered while using it, the circumstances of these incidents, your reactions, and their consequences”(请分享你使用中遇到的负面事件、事件背景、你的反应及后果)，确保捕捉 “负面事件 - 应对 - 影响” 的完整链条。

---
layout: scrollonecols
titleEn: 研究对象：62 位瑞士可穿戴设备用户
---
...
## **访谈周期**
2019 年 11 月 —2021 年 8 月(跨时近 2 年，覆盖不同季节的使用场景，如冬季室内运动、夏季户外跑步)

## **实施形式**

所有 62 次访谈均为面对面进行，并全程录音(经受访者同意)，后续逐字转录为文本。平均 49 分钟 / 次，最短 22 分钟，最长 105 分钟(因用户叙事详略、负面事件数量差异导致)。 最终形成 707 页访谈转录文本，为后续 “分解 - 理论重构 - 溯因” 分析提供充足数据基础。

## **出现重复即停止**

研究以 “理论类别饱和” 为停止招募的依据：“When themes and instances began to repeat, the theoretical categories were deemed to be sufficiently saturated (Urquhart et al. 2010)”—— 即当新招募的受访者讲述的 “负面事件类型”(如设备没电、数据不准、强制目标)、“应对方式”(如作弊、调整使用习惯、停用)、“情绪反应”(如沮丧、焦虑、自我苛责)开始与已有案例重复，且无新类别出现时，停止招募，确保数据足够支撑理论构建，同时避免冗余。

## **核心启发**

- ***长期模式***：捕捉用户 6 个月以上的 “有效使用演变”(如从 “严格遵循设备目标” 到 “选择性使用”)；

- ***主观体验***：还原用户面对负面事件的真实情绪(如 “设备没电导致数据丢失” 引发的 “沮丧” 与 “自我怀疑”)；

- ***因果链条***：清晰呈现 “负面事件→情绪→应对方式→有效使用变化” 的完整逻辑(如 “GPS 不准→愤怒→校准 GPS→数据准确性提升”)。
...
研究采用批判现实主义视角(Bhaskar 1975；Wynn and Williams 2012)，核心主张是 “现象背后存在不可直接观察的因果结构(生成机制、情境条件)，数据分析需揭示这些结构如何驱动可观察事件”。这一视角适配研究目标 —— 因为用户 “应对负面事件” 的行为不仅是表面现象，其背后存在 “为何选择该应对方式” 的深层逻辑(如用户的使用目标、设备的能动性)，需通过理论推演而非单纯描述现象来挖掘。

原文明确指出：“The strategy for data analysis followed established recommendations for forming critical realist explanations (Bhaskar 1975, Wynn and Williams 2012, Leidner et al. 2018) and to move conjecture into theory.” 即数据分析的核心是将 “对现象的猜想” 转化为 “有因果解释力的理论”。

## **迭代式分析流程**
数据分析并非一次性完成，而是从部分访谈数据开始，逐步新增数据与访谈案例，随理论类别(如应对方式、生成机制)的饱和不断完善。原文强调：“The approach entailed three steps—decomposition, theoretical redescription, and retroduction—and was iterative, starting with an initial subset of the interviews and adding more data and additional interviews as the theoretical categories developed.”
...
# **记忆偏差**
叙事访谈依赖于参与者的回忆，可能存在偏差 。

# **样本局限**

样本全部来自瑞士，文化背景单一，结论推广到其他文化背景需谨慎 。

# **结果变量单一**

研究主要关注“有效使用”这一指标，未直接测量心理健康或其他福祉指标 。

::right::

“One limitation is that, although narrative interviews can provide valuable insights, the possibility of memory bias cannot be ruled out.”(6. Limitations and Further Research)

“This study gathered data from a sample of 62 users of wearables from Switzerland but did not determine the extent to which its findings can be transferred to other contexts. Future research could determine whether the coping strategies and their consequences for effective use that were identified are also present in the use of other kinds of IS and agentic systems.”(6. Limitations and Further Research)

“Moreover, the scope of the present study is limited to how coping with negative incidents influences wearables’ effective use. Although effective use is a useful measure for assessing coping outcomes to negative incidents related to the use of wearables, the study does not assess them in other domains, such as psychological well-being.”(6. Limitations and Further Research)

“The study revealed some instances in which IS use is so effective that it becomes a negative incident in itself (e.g., by causing addiction), so the user has to reduce usage, thus lowering its effectiveness. Looking at patterns of over-effective use and technology dependency could be a promising pathway for future research.”(6. Limitations and Further Research)
